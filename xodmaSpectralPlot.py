...
from xodmaSpectralUtil import ParameterError
...
class NoteFormatter(Formatter):
    ...
    def __call__(self, x, pos=None):
        ...
        return hz_to_note(int(x), octave=self.octave, cents=cents)[0]
...
class ChromaFormatter(Formatter):
    ...
    def __call__(self, x, pos=None):
        ...
        return midi_to_note(int(x), octave=False, cents=False)
...
def __envelope(x, hop):
    ...
    return frame(x, hop_length=hop, frame_length=hop).max(axis=0)


def waveplot(y, sr=48000, max_points=5e4, x_axis='time', offset=0.0, max_sr=1000,
             **kwargs):
    ...
    locs = offset + frames_to_time(np.arange(len(y_top)),
                                        sr=sr,
...
def __coord_fft_hz(n, sr=48000, **_kwargs):
    ...
    fftFreqCoords = np.append(fftFreqs, (fftFreqs[-1] + (fftFreqs[-1] - fftFreqs[-2])))
    # return fft_frequencies(sr=sr, n_fft=1+n_fft)
    return fftFreqCoords
...
def __coord_mel_hz(n, fmin=0, fmax=11025.0, **_kwargs):
    ...
    return mel_frequencies(n+2, fmin=fmin, fmax=fmax)[1:]
...
def __coord_cqt_hz(n, fmin=None, bins_per_octave=12, **_kwargs):
    ...
    if fmin is None:
        fmin = note_to_hz('C1')
    return cqt_frequencies(n+1, fmin=fmin, bins_per_octave=bins_per_octave)
...
def __coord_tempo(n, sr=48000, hop_length=512, **_kwargs):
    '''Tempo coordinates'''
    return tempo_frequencies(n+1, sr=sr, hop_length=hop_length)
...
def __coord_time(n, sr=48000, hop_length=512, **_kwargs):
